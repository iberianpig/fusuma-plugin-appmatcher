    ...
    RSpec.describe Appmatcher do
      ...
      describe "#backend_klass" do
        ...
        context "when XDG_SESSION_TYPE is wayland" do
          ...
          context "when XDG_CURRENT_DESKTOP is KDE" do
            before { allow(Appmatcher).to receive(:xdg_current_desktop).and_return("KDE") }

            context "when kde-script is enabled" do
              before do
                allow_any_instance_of(Appmatcher::KwinScripts::Installer).to receive(:enabled?).and_return(true)
              end
              it { is_expected.to eq Appmatcher::KdeWayland }
            end

            context "when kde-script is NOT enabled" do
              before do
                allow_any_instance_of(Appmatcher::KwinScripts::Installer).to receive(:enabled?).and_return(false)
              end
              it { is_expected.to eq Appmatcher::UnsupportedBackend }
            end
          end
